...
if ozon_finance_by_posting is not None:
    st.caption(
        ...
        "Считается только по ДОСТАВЛЕННЫМ заказам (как «Продажи» в самом Ozon) — «В пути» смотри в таблице ниже. "
        "Итог может немного отличаться от «Финансовой сводки Ozon» выше: для самых свежих доставок (обычно "
        "последние дни периода) Ozon ещё не провёл финансовую операцию — по ним временно используется оценка."
    )
else:
